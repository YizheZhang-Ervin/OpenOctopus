<template>
  <div class="skills-page">
    ...
    <div class="full-page-container">
      ...
      <div v-if="loadingSkills" class="loading">Loading skills...</div>
...
export default {
  ...
  methods: {
    async loadSkills() {
      ...
      try {
        // Call Nanobot HTTP API to get skills list
        ...
        this.errorSkills =
          "Failed to load skills. Please make sure the Nanobot service is running.";
      } finally {
    ...
    formatRequirements(requires) {
      // Format skill requirements into user-friendly string
      ...
      if (requires.bins && requires.bins.length > 0) {
        parts.push(`Command line: ${requires.bins.join(", ")}`);
      }

      if (requires.env && requires.env.length > 0) {
        parts.push(`Environment variables: ${requires.env.join(", ")}`);
      }
